 ...
 * @module components/molecules/ErrorBoundary/__tests__/ErrorBoundary.test
 */
...
describe("ErrorBoundary", () => {
  ...
  it("should call Sentry.ErrorBoundary with ErrorBoundaryView when no fallback provided", () => {
    render(
      <ErrorBoundary>
        <Text>Child</Text>
      </ErrorBoundary>
    ...
    expect(Sentry.ErrorBoundary).toHaveBeenCalled();
    const callArgs = (Sentry.ErrorBoundary as jest.Mock).mock.calls[0][0];
    expect(callArgs.fallback.type).toBe(ErrorBoundaryView);
  });
});
