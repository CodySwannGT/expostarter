...
export function useBreakpointValue(values: BreakPointValue): any {
  ...
  const lastValuesStringRef = useRef<string>();
  ...
  useEffect(() => {
    if (typeof values === 'object') {
      ...
      const valuesChanged = valuesString !== lastValuesStringRef.current;
      const widthChanged = width !== lastWidthRef.current;

      if (valuesChanged || widthChanged) {
        lastValuesStringRef.current = valuesString;
        lastWidthRef.current = width;
        valuesRef.current = values;
        
        const finalBreakPointResolvedValue = getBreakPointValue(values, width);
        
        // Only update state if the calculated value actually changed
        setCurrentBreakPointValue((prev: any) => {
          if (JSON.stringify(prev) !== JSON.stringify(finalBreakPointResolvedValue)) {
            return finalBreakPointResolvedValue;
          }
          return prev;
        });
